...
Why COVID-19 and Normal Detection Succeeded
...
[Placeholder: Include the fine-tuned SigLIP confusion matrix figure showing the detailed breakdown of predictions vs true labels]
Proposed Improvements
Several strategies could address the observed limitations and improve overall classification accuracy in future iterations.
...
Second, implementing specialized few-shot or meta-learning techniques designed for rare class detection could help the model learn from limited tuberculosis examples more effectively. Prototypical networks or matching networks that learn to compare new images against class prototypes might achieve better tuberculosis recognition with fewer training samples.
Third, incorporating multi-task learning with auxiliary objectives could improve feature learning. Training the model to simultaneously predict disease severity, affected lung regions, or imaging modality alongside the primary classification task might encourage learning of more generalizable representations.
Fourth, addressing the modality gap between X-ray and CT images through domain adaptation techniques could improve cross-modality generalization. Training separate pathway heads for different imaging modalities or using domain-adversarial training to learn modality-invariant features are potential approaches.
Fifth, ensembling multiple model architectures could leverage complementary strengths. Combining the fine-tuned SigLIP model with other architectures such as ResNet, EfficientNet, or specialized medical imaging models like CheXNet might improve overall robustness and reduce class-specific blind spots.
Finally, acquiring additional high-quality tuberculosis imaging data from medical repositories or clinical partnerships would directly address the fundamental data scarcity problem. Given that tuberculosis remains a significant global health concern with extensive medical imaging archives, expanding the tuberculosis subset to achieve better class balance represents the most straightforward path to improved detection accuracy.

**5.4 Conclusion**
The final evaluation on held-out test data demonstrates that fine-tuning the SigLIP architecture substantially improves medical image classification performance compared to zero-shot approaches, achieving 62.61% UAR versus 33.9% for Qwen2.5-VL-3B and 33.33% for baseline SigLIP. The model excels at distinguishing COVID-19 from normal lung presentations but fails entirely on tuberculosis detection due to severe class imbalance in the training data. Future work should prioritize addressing this imbalance through data augmentation, synthetic sample generation, and acquisition of additional tuberculosis imaging data to achieve clinically viable multi-class respiratory disease classification.


For the placeholders, should include:
The side-by-side confusion matrix comparison (Fig. 2 from your report showing baseline vs fine-tuned SigLIP)
2-3 example TB images that were misclassified - you can pull these from your test set predictions CSV
The detailed fine-tuned SigLIP confusion matrix

If you want, I can help you identify specific misclassified images from your data or adjust any section's tone/length.Claude is AI and can make mistakes. Please double-check responses.
